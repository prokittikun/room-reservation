...
switch ($route) {
    case '/contact/save':
        $params = [
            $id, $company_name, $house_no, $village_no, $village_name,
            $alley, $junction, $road, $province, $district, $sub_district, $email,
            $tel, $create_at, $update_at
        ];
        ...
        try {
            ...
            if ($stmt->rowCount() > 0) {
                ...
                $params = [
                    $company_name, $house_no, $village_no, $village_name,
                    $alley, $junction, $road, $province, $district, $sub_district, $email,
                    $tel, $update_at, $id
                ];
                $sql = "UPDATE contact_us SET company_name=?,house_no=?,";
                $sql .= "village_no=?,village_name=?,alley=?,junction=?,";
                $sql .= "road=?,province=?,district=?,sub_district=?,";
                $sql .= "email=?,tel=?,update_at=? WHERE id=?";
            }
            echo $sql;
            $stmt = connect_db()->prepare($sql);
            $stmt->execute($params);
