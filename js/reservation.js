...
$('#reservationBtn').click(function () {
    ...
    if (!is_login) {
        ...
        const state = `id=${id}#reservationForm`
...
$('#slipPayment').change(function () {
    ...
    $('#slipPaymentPreview').html(postponeSlipPaymentEl)
})
...
$('#reservationHandleSubmit').click(function () {
    ...
    validateCount += is_slip ? 1 : 0


    if (validateCount == 0) {
        const formData = new FormData();
        formData.append("route", '/booking/slippayment')
        formData.append("id", atob($('#reservationHandleSubmit').attr('data-id')))
        ...
        $.ajax({
            ...
            complete: function (xhr, textStatus) {
                if (xhr.status == 200) {
                    ...
                    errDialog('แจ้งเตือน', 'การชำระเงินล้มเหลว', '')
                }
            }
        })
    }
