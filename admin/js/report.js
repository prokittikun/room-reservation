...
$('button[name="report-to-file"]').click(function () {
  ...
  const pay_status = document.getElementById("payStatus").value;
  const status = getStatus().val();
  if (!start && !end) {
        ...
        try {
          ...
          if (xhr.status == 200) {
            ...
            if (is_items == false) {
              ...
              msg +=
                "โปรดลดจำนวนวันที่สำหรับการออกรายงาน เพื่อไม่เกินจำนวนที่กำหนด ";
              msg +=
                "หรือให้ออกเป็นแบบเฉลี่ยเป็นรอบ เพื่อให้ได้รายงานครบตามวันที่ต้องการ";
            }
...
$("#filterByData").click(function () {
  ...
  if (sort == "all") {
    errDialog(
      "การจัดเรียง",
      "หากต้องการกรองข้อมูลตามที่เลือกโปรดเลือกการจัดเรียงแบบอื่น"
    );
  }
  if (sort && sort != "all") {
