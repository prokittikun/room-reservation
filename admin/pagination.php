...
function create_pagination($index, $count, $route, $row_all, $idx_start, $idx_end)
{
    ...
    if ($row_all > 0) {
        ...
        $pagination .= '<strong class="font-weight-bold mx-1 text-muted">' . $idx_start . '</strong>';
        $pagination .= '<span>ถึง</span>';
        $pagination .= '<strong class="font-weight-bold mx-1 text-muted">' . $idx_end . '</strong>';
    }


    $pagination .= '<span>ทั้งหมด</span>';
    $pagination .= "<strong class='text-lightblue mx-1'>$row_all</strong>";
    $pagination .= '<span>รายการ</span>';
    $pagination .= '</div>';
